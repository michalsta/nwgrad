...
//   1. set_problem(seq_a, seq_b, matrix, gap_extend[, gap_open[, band[, guide_j]]])
...
struct Aligner {
    ...
    void hard_grad(double grad[256][256]) const {
        check_viterbi();
        hard_grad(own_buf_, grad);
    }

    void soft_grad(double grad[256][256]) const {
        check_fwdbwd();
    ...
    void hard_grad(const DpBuffer& buf, double grad[256][256]) const {
        if constexpr (GM == GapModel::Linear) hard_grad_linear(buf, grad);
        else                                   hard_grad_affine(buf, grad);
    }

    void soft_grad(const DpBuffer& buf, double grad[256][256]) const {
...
private:
    ...
    std::vector<int> guide_j_linear(const DpBuffer& buf) const {
        ...
        while (true) {
            ...
            if (i > 0 && j > 0 &&
                rat(buf.H, i, j) == rat(buf.H, i-1, j-1) + matrix_->score(seq_a_[i-1], seq_b_[j-1]))
            {
                --i; --j;
                gj[static_cast<size_t>(i)] = j;
            } else if (i > 0 && rat(buf.H, i, j) == rat(buf.H, i-1, j) - gap_extend_) {
                --i;
                gj[static_cast<size_t>(i)] = j;
            } else {
    ...
    std::vector<int> guide_j_affine(const DpBuffer& buf) const {
        ...
        while (true) {
            ...
            if (tbl == TBTable::M) {
                ...
                double fm = rat(buf.VM,i-1,j) - gap_open_ - gap_extend_;
                double fx = rat(buf.VX,i-1,j) - gap_extend_;
                double fy = rat(buf.VY,i-1,j) - gap_open_ - gap_extend_;
                --i;
                gj[static_cast<size_t>(i)] = j;
                ...
                double fm = rat(buf.VM,i,j-1) - gap_open_ - gap_extend_;
                double fx = rat(buf.VX,i,j-1) - gap_open_ - gap_extend_;
                double fy = rat(buf.VY,i,j-1) - gap_extend_;
                --j;  // gap in a: i stays, no gj update
                if      (fm >= fx && fm >= fy) tbl = TBTable::M;
                else if (fx >= fy)             tbl = TBTable::X;
                else                            tbl = TBTable::Y;
    ...
    void viterbi_linear(DpBuffer& buf) {
        ...
        if constexpr (AM == AlignMode::Global) {
            ...
            for (int i = 1; i <= bi; ++i) at(buf.H, i, 0) = -i * gap_extend_;
            for (int j = 1; j <= bj; ++j) at(buf.H, 0, j) = -j * gap_extend_;
        } else {
        ...
        for (int i = 1; i <= m_; ++i) {
            for (int j = jlo(i); j <= jhi(i); ++j) {
                double v = std::max({
                    rat(buf.H, i-1, j-1) + matrix_->score(seq_a_[i-1], seq_b_[j-1]),
                    rat(buf.H, i-1, j)   - gap_extend_,
                    rat(buf.H, i,   j-1) - gap_extend_,
                });
                if constexpr (AM == AlignMode::Local) {
                    v = std::max(v, 0.0);
    ...
    void hard_grad_linear(const DpBuffer& buf, double grad[256][256]) const {
        int i = (AM == AlignMode::Global) ? m_ : best_i_;
        int j = (AM == AlignMode::Global) ? n_ : best_j_;
        ...
        while (true) {
            ...
            if (i > 0 && j > 0 &&
                rat(buf.H, i, j) == rat(buf.H, i-1, j-1) + matrix_->score(seq_a_[i-1], seq_b_[j-1]))
            {
                grad[static_cast<unsigned char>(seq_a_[i-1])]
    ...
    void viterbi_affine(DpBuffer& buf) {
        ...
        if constexpr (AM == AlignMode::Global) {
            at(buf.VM, 0, 0) = 0.0;
            const int bi = border_rows(), bj = border_cols();
            for (int i = 1; i <= bi; ++i) at(buf.VX, i, 0) = -(gap_open_ + i * gap_extend_);
            for (int j = 1; j <= bj; ++j) at(buf.VY, 0, j) = -(gap_open_ + j * gap_extend_);
        } else {
        ...
        for (int i = 1; i <= m_; ++i) {
            for (int j = jlo(i); j <= jhi(i); ++j) {
                ...
                double m_val = diag + matrix_->score(seq_a_[i-1], seq_b_[j-1]);
                double x_val = std::max({rat(buf.VM,i-1,j) - gap_open_ - gap_extend_,
                                         rat(buf.VX,i-1,j)             - gap_extend_,
                                         rat(buf.VY,i-1,j) - gap_open_ - gap_extend_});
                double y_val = std::max({rat(buf.VM,i,j-1) - gap_open_ - gap_extend_,
                                         rat(buf.VX,i,j-1) - gap_open_ - gap_extend_,
                                         rat(buf.VY,i,j-1)             - gap_extend_});
    ...
    void hard_grad_affine(const DpBuffer& buf, double grad[256][256]) const {
        traceback_affine_impl(buf, [&](int i, int j) {
            grad[static_cast<unsigned char>(seq_a_[i])]
                [static_cast<unsigned char>(seq_b_[j])] += 1.0;
        });
    }
    ...
    void fwdbwd_linear(DpBuffer& buf) {
        ...
        if constexpr (AM == AlignMode::Global) {
            at(buf.F, 0, 0) = 0.0;
            const int bi = border_rows(), bj = border_cols();
            for (int i = 1; i <= bi; ++i) at(buf.F, i, 0) = -static_cast<double>(i) * gap_extend_;
            for (int j = 1; j <= bj; ++j) at(buf.F, 0, j) = -static_cast<double>(j) * gap_extend_;
        } else {
        ...
        for (int i = 1; i <= m_; ++i) {
            for (int j = jlo(i); j <= jhi(i); ++j) {
                double v = lse3(
                    rat(buf.F, i-1, j-1) + matrix_->score(seq_a_[i-1], seq_b_[j-1]),
                    rat(buf.F, i-1, j)   - gap_extend_,
                    rat(buf.F, i,   j-1) - gap_extend_
                );
        ...
        for (int i = m_; i >= 0; --i) {
            for (int j = jhi0(i); j >= jlo0(i); --j) {
                ...
                if (i > 0 && j > 0) at(buf.B,i-1,j-1) = lse2(rat(buf.B,i-1,j-1), bval + matrix_->score(seq_a_[i-1], seq_b_[j-1]));
                if (i > 0)          at(buf.B,i-1,j)   = lse2(rat(buf.B,i-1,j),   bval - gap_extend_);
                if (j > 0)          at(buf.B,i,j-1)   = lse2(rat(buf.B,i,j-1),   bval - gap_extend_);
            }
        }
    }

    void soft_grad_linear(const DpBuffer& buf, double grad[256][256]) const {
        for (int i = 1; i <= m_; ++i) {
            for (int j = jlo(i); j <= jhi(i); ++j) {
                double bval = rat(buf.B, i, j);
                if (bval == NEG_INF) continue;
                double log_p = rat(buf.F, i-1, j-1) + matrix_->score(seq_a_[i-1], seq_b_[j-1]) + bval - log_z_;
                grad[static_cast<unsigned char>(seq_a_[i-1])]
    ...
    void fwdbwd_affine(DpBuffer& buf) {
        ...
        if constexpr (AM == AlignMode::Global) {
            at(buf.FM, 0, 0) = 0.0;
            const int bi = border_rows(), bj = border_cols();
            for (int i = 1; i <= bi; ++i) at(buf.FX, i, 0) = -(gap_open_ + static_cast<double>(i) * gap_extend_);
            for (int j = 1; j <= bj; ++j) at(buf.FY, 0, j) = -(gap_open_ + static_cast<double>(j) * gap_extend_);
        } else {
        ...
        for (int i = 1; i <= m_; ++i) {
            for (int j = jlo(i); j <= jhi(i); ++j) {
                ...
                double m_val = diag + matrix_->score(seq_a_[i-1], seq_b_[j-1]);
                double x_val = lse3(rat(buf.FM,i-1,j) - gap_open_ - gap_extend_,
                                    rat(buf.FX,i-1,j)             - gap_extend_,
                                    rat(buf.FY,i-1,j) - gap_open_ - gap_extend_);
                double y_val = lse3(rat(buf.FM,i,j-1) - gap_open_ - gap_extend_,
                                    rat(buf.FX,i,j-1) - gap_open_ - gap_extend_,
                                    rat(buf.FY,i,j-1)             - gap_extend_);
        ...
        for (int i = m_; i >= 0; --i) {
            for (int j = jhi0(i); j >= jlo0(i); --j) {
                ...
                if (bm != NEG_INF && i > 0 && j > 0) {
                    double contrib = bm + matrix_->score(seq_a_[i-1], seq_b_[j-1]);
                ...
                if (bx != NEG_INF && i > 0) {
                    at(buf.BM,i-1,j) = lse2(rat(buf.BM,i-1,j), bx - gap_open_ - gap_extend_);
                    at(buf.BX,i-1,j) = lse2(rat(buf.BX,i-1,j), bx - gap_extend_);
                    at(buf.BY,i-1,j) = lse2(rat(buf.BY,i-1,j), bx - gap_open_ - gap_extend_);
                }
                if (by != NEG_INF && j > 0) {
                    at(buf.BM,i,j-1) = lse2(rat(buf.BM,i,j-1), by - gap_open_ - gap_extend_);
                    at(buf.BX,i,j-1) = lse2(rat(buf.BX,i,j-1), by - gap_open_ - gap_extend_);
                    at(buf.BY,i,j-1) = lse2(rat(buf.BY,i,j-1), by - gap_extend_);
                }
            }
        }
    }

    void soft_grad_affine(const DpBuffer& buf, double grad[256][256]) const {
        for (int i = 1; i <= m_; ++i) {
            for (int j = jlo(i); j <= jhi(i); ++j) {
                double bm = rat(buf.BM, i, j);
                if (bm == NEG_INF) continue;
                double pred  = lse3(rat(buf.FM,i-1,j-1), rat(buf.FX,i-1,j-1), rat(buf.FY,i-1,j-1));
                double log_p = pred + matrix_->score(seq_a_[i-1], seq_b_[j-1]) + bm - log_z_;
                grad[static_cast<unsigned char>(seq_a_[i-1])]
                    [static_cast<unsigned char>(seq_b_[j-1])] += std::exp(log_p);
            }
        }
